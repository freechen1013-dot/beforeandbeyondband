...
export default function HomePage() {
  ...
  const slideImages = slides.length > 0
    ? slides.map((s) => urlFor(s.image).width(1920).url())
    : fallbackSlides
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {posts.map((post) => (
                <Link
                  ...
                  className="block bg-[#000060]/50 border border-blue-900/40 rounded-xl p-6 hover:border-blue-500/50 transition-colors"
                >
